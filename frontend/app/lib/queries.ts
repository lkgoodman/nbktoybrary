...
export function useUpdateCheckoutDueDate(): UseMutationResult<CheckoutRead, Error, { id: string; dueAt: string; token: string }> {
  return useMutation<CheckoutRead, Error, { id: string; dueAt: string; token: string }>({
    mutationFn: ({ id, dueAt, token }) => updateCheckoutDueDate(id, dueAt, token),
  });
}
