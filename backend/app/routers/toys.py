...
_load_images = selectinload(Toy.images)
...
async def create_toy(payload: ToyCreate, db: AsyncSession = Depends(get_db)) -> Toy:
    ...
    db.add(toy)
    await db.commit()
    result = await db.execute(select(Toy).options(_load_images).where(Toy.id == toy.id))
...
async def list_toys(db: AsyncSession = Depends(get_db)) -> list[Toy]:
    result = await db.execute(
        select(Toy).options(_load_images).order_by(Toy.created_at.desc())
    )
...
async def get_toy(toy_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> Toy:
    result = await db.execute(
        select(Toy).options(_load_images).where(Toy.id == toy_id)
    )
